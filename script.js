...
function fetchGreetings() {
    fetch('data.json')
        .then(response => response.json())
        .then(data => {
            // Depending on the time of day, send a specific greeting
            let hours = new Date().getHours();
            if (hours < 12) {
                speak(data.greetings.morning);
            } else if (hours < 18) {
                speak(data.greetings.afternoon);
            } else {
                speak(data.greetings.evening);
            }
        });
}
...
function fetchXMLGreetings() {
    fetch('data.xml')
        ...
        .then(data => {
            ...
            let morning = xmlDoc.getElementsByTagName("morning")[0].childNodes[0].nodeValue;
            speak(morning);
        });
}
...
function takeCommand(message) {
    ...
    // if (message.includes("viras") || message.includes("hey viras") || message.includes("hello viras")) {
    ...
    // New function to tell a joke
        ...
        speak("Sure! Here's a joke for you: Why don't skeletons fight each other? They don't have the guts!")
    }
    ...
    // New function to read a motivational quote
        ...
        speak("Here is a motivational quote for you: 'The only way to do great work is to love what you do.' - Steve Jobs")
    }
    ...
    else if (message.includes("translate") || message.includes("in spanish") || message.includes("in french")) {
        let textToTranslate = message.replace("translate", "").replace("in spanish", "").replace("in french", "").trim();
        if (textToTranslate) {
            speak(`Translating "${textToTranslate}" into Spanish...`)
    ...
    // }
        ...
        if (message.includes("highest wicket taker") || message.includes("cricket")) {
        ...
        // For any other unmatched queries, fallback to a general search
            ...
            speak("I couldn't understand that, but let me find something for you...");
            window.open(`https://www.google.com/search?q=${message}`, "_blank");
        }
    }
}
